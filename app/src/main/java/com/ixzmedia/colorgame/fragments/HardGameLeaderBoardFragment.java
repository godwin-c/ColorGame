...
public class HardGameLeaderBoardFragment extends Fragment {

    SwipeRefreshLayout frag_hard_swipe_refresh;
    RecyclerView frag_hard_recycler_view;

    TextView empty_data_view;

    private static final String HARD_LEVEL = "hard_level";

    private static final String TAG = HardGameLeaderBoardFragment.class.getSimpleName();

    public HardGameLeaderBoardFragment() {
        // Required empty public constructor
    }


    // TODO: Rename and change types and number of parameters
    public static HardGameLeaderBoardFragment newInstance(String param1, String param2) {
        HardGameLeaderBoardFragment fragment = new HardGameLeaderBoardFragment();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(R.layout.fragment_hard_game_leader_board, container, false);

        frag_hard_swipe_refresh = (SwipeRefreshLayout) view.findViewById(R.id.frag_hard_swipe_refresh);
        frag_hard_recycler_view = (RecyclerView) view.findViewById(R.id.frag_hard_recycler_view);
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        if (fileExist(HARD_LEVEL)){
            Log.d(TAG, "onCreate: File Exists");

            ArrayList<HighScoreModelResponse> easyHighScores = readFromStorage(HARD_LEVEL);

            frag_hard_recycler_view.setVisibility(View.VISIBLE);
            empty_data_view.setVisibility(View.GONE);

            setupRecyclerView(easyHighScores);
        } else {
            frag_hard_recycler_view.setVisibility(View.GONE);
    ...
    private void setActionsOnViewItems() {
        ...
        frag_hard_swipe_refresh.setOnRefreshListener(new SwipeRefreshLayout.OnRefreshListener() {
            @Override
        ...
        frag_hard_swipe_refresh.setColorSchemeResources(
...
//                            medium
                            frag_hard_swipe_refresh.setRefreshing(false);
    ...
    private void setupDataForDisplay(ArrayList<HighScoreModelResponse> highScoreModelResponses) {
        ...
        //restartActivity();
        setupRecyclerView(hardHighScore);
    }
    ...
    private void setupRecyclerView(ArrayList<HighScoreModelResponse> easyHighScores) {
        ...
        if (frag_hard_recycler_view.getAdapter() == null || frag_hard_recycler_view.getAdapter().getItemCount() < 1){
            ...
            frag_hard_recycler_view.addItemDecoration(new VerticalSpace(20));
            frag_hard_recycler_view.setAdapter(adapter);
        }else {
            adapter = (HighScoreAdapter) frag_hard_recycler_view.getAdapter();
